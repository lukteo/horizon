...
func (h *Handler) ListApiKeys(
	...
	keys, err := h.orgSvc.ListAPIKeys(ctx, request.OrgId)
	if err != nil {
		return nil, err
	}
	if keys == nil {
...
func (h *Handler) CreateApiKey(
	...
	key, err := h.orgSvc.CreateAPIKey(ctx, request.OrgId, request.Body.Name, request.Body.Scopes)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) RevokeApiKey(
	...
	if err := h.orgSvc.RevokeAPIKey(ctx, request.OrgId, request.KeyId); err != nil {
		if errors.Is(err, orgservice.ErrNotFound) {
